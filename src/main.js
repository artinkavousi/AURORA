...
class App {
    ...
    async init(progressCallback) {
        ...
        const routerScene = this.router?.getScene?.();
        if (conf.audioScene && routerScene && conf.audioScene !== routerScene) {
            this.router.setScene(conf.audioScene, conf);
        }
        if (!conf.audioScene) {
            conf.audioScene = this.router?.getScene?.() || 'Neon Pulse';
        } else if (this.router?.getScene) {
            conf.audioScene = this.router.getScene();
        }
        this.audioPanel = new AudioPanel(this.audio, conf, this.router);
        this.audioPanel.init('left');
